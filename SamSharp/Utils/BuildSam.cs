...
namespace SamSharp.Utils
{
	...
	{
		internal static Sam BuildSamModel(string checkpoint, Device device)
		{
			List<CommonTensor> commonTensors = PickleLoader.ReadTensorsInfoFromFile(checkpoint);
			return commonTensors.Count switch
			{
				314 => build_sam_vit_b(checkpoint,device),
				594 => build_sam_vit_h(checkpoint, device),
				482 => build_sam_vit_l(checkpoint, device),
				_ => throw new ArgumentException("Invalid SAM type specified.")
			};
		}

		private static Sam build_sam_vit_h(string checkpoint, Device device)
		{
			return _build_sam(
		...
		private static Sam build_sam_vit_l(string checkpoint, Device device)
		{
			return _build_sam(
		...
		private static Sam build_sam_vit_b(string checkpoint, Device device)
		{
			return _build_sam(
		...
		private static Sam _build_sam(int encoder_embed_dim, int encoder_depth, int encoder_num_heads, int[] encoder_global_attn_indexes, string? checkpoint = null, Device? device = null)
		{
			...
			device = device is null ? new Device(DeviceType.CPU) : CUDA;
			...
			MaskDecoder maskDecoder = new MaskDecoder(
				...
				iou_head_depth: 3,
				iou_head_hidden_dim: 256).to(device);
